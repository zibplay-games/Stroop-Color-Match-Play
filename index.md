# Privacy Policy for Word Search Play

**Last updated:** 15 June 2026

This Privacy Policy explains how **ZibPlay** ("we", "us", or "our") collects, uses, and shares information when you use the mobile game **Word Search Play**.

By using the game, you agree to this Privacy Policy.

## 1. Information We Collect

Word Search Play does not require account registration.

We collect limited information necessary to operate, improve, secure, and maintain the game.

This may include:

* **Name:** if you contact us or submit a support request.
* **Email address:** if you contact us or submit a support request.
* **Approximate location:** derived from IP address or platform services, used for security, analytics, diagnostics, or service operation.
* **Diagnostics:** crash logs, error reports, loading performance, technical logs, and other diagnostic information.
* **App interactions:** gameplay activity, game sessions, screens viewed, taps, selections, and basic interaction events.
* **Other actions:** in-game actions, settings changes, progress events, and other activity related to how you use the game.
* **Device or other IDs:** device identifiers, installation identifiers, or other technical identifiers used for analytics, diagnostics, security, or service operation.

We do not require users to create an account, and we do not intentionally collect sensitive personal information.

## 2. Advertising

Word Search Play does not display third-party advertising.

We do not use advertising data to show ads inside the game.

## 3. How We Use Information

We may use information to:

* Operate and maintain the game.
* Improve gameplay, stability, and performance.
* Fix bugs and technical issues.
* Understand basic usage of the game.
* Prevent fraud, abuse, and security incidents.
* Respond to support requests or user inquiries.
* Comply with legal obligations.

## 4. Third-Party Services

The game may use third-party services for app distribution, hosting, analytics, crash reporting, diagnostics, security, or technical functionality.

These third-party services may process information according to their own privacy policies.

Examples may include:

* Google Play services
* Firebase, if analytics or crash reporting is enabled
* Hosting, analytics, security, crash reporting, or technical service providers used to operate and maintain the game

## 5. Gameplay Data

Word Search Play may store basic gameplay data such as scores, progress, settings, preferences, or in-game activity.

This data may be stored locally on your device or processed through technical services used to operate and improve the game.

## 6. Data Sharing

We do not sell your personal information.

We may share or allow processing of limited information only when necessary, including:

* With service providers used to operate, analyze, secure, diagnose, or maintain the game.
* With platform providers, such as Google Play services.
* With authorities if required by law.
* In connection with a business transfer, such as a merger, acquisition, or sale of assets.

The types of information that may be shared or processed include:

* Name, if provided through a support request.
* Email address, if provided through a support request.
* Approximate location.
* Diagnostics.
* App interactions.
* Other in-game actions.
* Device or other IDs.

## 7. Data Retention and Deletion

We retain information only for as long as necessary for the purposes described in this Privacy Policy.

Some gameplay data may remain stored locally on your device unless you delete the game or clear the app data.

To request deletion of data related to your use of the game, contact us at:

**[privacy@zibplay.com](mailto:privacy@zibplay.com)**

## 8. Children's Privacy

Word Search Play is not intended to knowingly collect personal information from children.

If you believe that a child has provided personal information, please contact us at **[privacy@zibplay.com](mailto:privacy@zibplay.com)** and we will review and delete the information where required.

## 9. Security

We use reasonable technical and organizational measures to protect information processed through the game.

However, no method of transmission or electronic storage is fully secure.

## 10. Your Rights

Depending on your location, you may have rights to access, correct, delete, restrict, or object to the processing of your personal information.

To exercise these rights, contact us at:

**[privacy@zibplay.com](mailto:privacy@zibplay.com)**

## 11. Changes to This Privacy Policy

We may update this Privacy Policy from time to time.

Any changes will be posted on this page with an updated "Last updated" date.

## 12. Contact

For privacy questions, contact us at:

**[privacy@zibplay.com](mailto:privacy@zibplay.com)**

For copyright complaints or abuse reports, contact:

**[abuse@zibplay.com](mailto:abuse@zibplay.com)**

For general contact, contact us at:

**[hello@zibplay.com](mailto:hello@zibplay.com)**
